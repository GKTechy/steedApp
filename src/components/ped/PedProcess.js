...
//        console.log('props profile-->:'+this.props.apiurl)
    ...
    saveClick= event =>{
        if(this.state.processName === ""){
        ...
        }else if(!this.state.isActive && this.state.processId===0){
            this.setState({
            ...
            const obj = {'processName':this.state.processName, isActive:tempstatus,'processId':this.state.processId,"updatedBy":this.state.loginUser.userId,"createdBy":this.state.loginUser.userId};

             // POST request using fetch with error handling
                const requestOptions = {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ "processObj": obj })
                };
                fetch(this.props.apiurl+"process/saveProcess", requestOptions)
                    .then(async response => {
...
const mapStateToProps = (state) => {
 ...
 export default connect(mapStateToProps)(PedProcess);
